...
expression to `--collect-files-filter`:

```
prebuild -t 8.0.0 --collect-files-filter "\.(node|a)$"
```
...
prebuild [options]
  ...
  --collect-files-filter        (regex to match files that will be distributed [default: '\.node$'])
